...
|---|---|---|---|
| `IsolatedStorage(Encrypted)`, `ENCRYPT`/`DECRYPT` (issue #66) | encrypted write reads back as plaintext | "An encryption key is required to complete the request." | **The image, and Patch #26's stated reason for existing is wrong — FIX, don't document.** The patch header says `CreateKey()` "depends on Windows-only key storage". It does not. `RsaEncryptionProviderBase.CreateKey()` (Nav.Core.dll) is `RSACryptoServiceProvider` + `ToXmlString` + `File.Create(name, 1024, FileOptions.Encrypted)` + a hash; both of those APIs work on .NET 8 Linux (verified directly). Measured with `BC_DISABLE_PATCHES=26`: the tier reports the *same message the Windows container reports*, AND a real 1682-byte RSA private key file is written to `/usr/share/Microsoft/Microsoft Dynamics NAV/280/Server/Keys/default_<guid>.key`, and `IsEncryptionEnabled()` flips to true afterwards. So key creation works; something later in the enable flow still reports "not created", and `BC_DEBUG_FIRSTCHANCE=1` shows no platform exception anywhere — only BC's own `NavEncryptionNotCreatedException`. Finish that diagnosis and Patch #26 can probably be deleted in favour of real encryption. Separate note: that Keys directory is NOT on a volume, so a container recreate loses the key. |
| `Media.ExportStream()` byte length (issue #67) | exactly the imported bytes (68 in, 68 out) | 31 bytes more (68 in, 99 out) | **The image.** BC's `NavMediaImage.Bytes()` re-encodes through `Image.Save()` when it holds no original stream; `DrawingStub`'s `Save()` writes the source bytes back verbatim instead (deliberately — a re-encoding stub is what issue #18 was). So a media round-trip is byte-preserving here and re-encoded on a real tier. **FIX, don't document.** "Byte-preservation is cleaner" is not a defence when the point of the tier is to tell you what BC does. `libSkiaSharp.so` and `SkiaSharp.dll` are already in `/bc/service` for the report renderer, so `DrawingStub` can decode and encode through Skia instead of writing `SourceBytes` back. That makes the observable behaviour match (a round-trip re-encodes, dimensions and format are real, resize works). It will not match GDI+ byte-for-byte, and that residual is worth documenting — but a test asserting equality with the import would then correctly fail on both tiers. Needs its own PR and a BCApps sweep: DrawingStub caused issue #18 and the ~17 "no content" failures. |
| Outbound-HTTP consent prompt (issue #68) | prompt fires; a `[StrMenuHandler]` answers it | no prompt; a declared handler goes unexecuted | **Settled, and the one row with no fix available.** The prompt is a sandbox protection: BC raises it in sandbox environments and never in production. It is driven by `TenantEnvironmentType`, and `entrypoint.sh` sets that to `Sandbox` unconditionally. That is not optional — measured: with `TenantEnvironmentType=Production` the platform test framework returns no results at all, on BOTH runners ("no test results returned for a Subtype=Test codeunit" on altool, "TestRunner API not available" on websocket). So any tier that can run AL tests here prompts. The Windows nightly avoids it only by using `onprem` artifacts. **This is a sandbox-vs-onprem difference, not Linux-vs-Windows** — a Windows *sandbox* container would prompt too. A corpus test that depends on consent behaviour is pinning the environment type. |
